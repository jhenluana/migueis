  ...
  async componentDidMount() {
    await api.get('/comanda/listar').then( response =>{
      ...
      function onlyUnique(value, index, self) {
        return self.indexOf(value.produto) === index;
      }
      
      // usage example:
      var unique = tmp3.filter(onlyUnique);
      console.log(unique);
      const countOccurrences = (arr, val) => arr.reduce((a, v) => (v.produto === val ? a + 1 : a), 0);
      console.log(countOccurrences(tmp3, "Pintado frito"));
