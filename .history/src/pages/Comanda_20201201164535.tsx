...
function Comanda() {
  ...
  const [comanda,setComanda] = useState(localStorage.getItem('comanda'));
  ...
  const makeRows = () => {
    ...
    console.log(tmp)
    if(tmp){
      const rows = tmp.map((p:any) => {return {nome:p.nome, qtd:p.qtd}});
      return rows;
    }
  }
  
  const rows = JSON.parse(localStorage.getItem('comanda') || '');
  return (
    <div className="page-pedidos">
      ...
      <Table className={classes.table} size="small" aria-label="a dense table">
        ...
        <TableBody>
          {(rows.length) > 0 && rows.map((row:any) => {row.qtd>0 && (
